"""REINFORCE comparison algorithm."""
...
class REINFORCEBaseline(DRLBaseline):
    """Monte Carlo policy-gradient baseline with pairwise legal actions."""
    ...
    def _sample_trajectory(self, instance: dict):
        ...
        while not done and step < max_steps:
            ...
            encoded = self.actor_encoder(state_tensor)
            action, _ = self.actor.select_action(
                encoded, state_tensor["edge_features"], pair_mask, deterministic=False
            )
            log_prob, entropy, _ = self.actor.get_action_log_prob(
                encoded, state_tensor["edge_features"], pair_mask, action
            )
    ...
    def _update(self, trajectories):
        ...
        for traj in trajectories:
            if not traj["rewards"]:
                continue
            all_returns.append(discounted_returns(traj["rewards"], self.gamma, self.device))
            all_log_probs.extend(traj["log_probs"])
            all_entropies.extend(traj["entropies"])

        if not all_log_probs:
            return {"actor_loss": 0.0, "entropy": 0.0}

        returns = normalize_tensor(torch.cat(all_returns))
        log_probs = torch.stack(all_log_probs)
        entropies = torch.stack(all_entropies)
        actor_loss = -(log_probs * returns).mean() - self.entropy_coef * entropies.mean()

        self.optimizer.zero_grad()
        actor_loss.backward()
        params = list(self.actor_encoder.parameters()) + list(self.actor.parameters())
    ...
    def train_epoch(self, instance: dict, cfg, epoch: int) -> dict:
        """One REINFORCE epoch: sample trajectories serially, then one update."""
    ...
    def train(self, instance: dict, cfg=None):
        """Train REINFORCE on one public instance."""
        set_seed(self.seed)
        start = now()
    ...
    def load(self, path: str):
        """Load REINFORCE checkpoint. Models must be built first via train() or _build()."""
